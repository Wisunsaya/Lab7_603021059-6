...
class MainActivity : AppCompatActivity() {
    ...
    fun addStudent(v: View){
        ...
        mDialogView.btnAdd.setOnClickListener{
            studentList.add( Student(mDialogView.edit_id.text.toString(), mDialogView.edt_name.text.toString(),
                mDialogView.edt_age.text.toString().toInt()))
            recycler_view.adapter?.notifyDataSetChanged()
            Toast.makeText(applicationContext,"The student id added successfully", Toast.LENGTH_SHORT).show()
        }
    ...
    fun testStudentData(){
        studentList.add(Student("603021059-6","Student 1", 20))
        studentList.add(Student("603021059-6","Student 2", 21))
    }
}
